...
#define PAGE_ROUNDUP(n) \
  ((n) % PAGE_SIZE == 0 ? (n) : ((n) + PAGE_SIZE) & ~(PAGE_SIZE - 1))
...
#define BUS_GPIO_REG(a) \
  (GPIO_BASE - PHYS_REG_BASE + BUS_REG_BASE + (uint32_t)(a))
#define GPIO_IN 0
#define GPIO_OUT 1
#define GPIO_ALT0 4
#define GPIO_ALT2 6
...
#define BUS_DMA_MEM(a) \
  ((uint32_t)a - (uint32_t)virt_dma_mem + (uint32_t)bus_dma_mem)
...
void dma_test_pwm_trigger(int pin) {
  ...
  cbs[0].ti = cbs[1].ti = cbs[2].ti = cbs[3].ti =
      (1 << 6) | (DMA_PWM_DREQ << 16);
...
uint32_t set_vc_clock(int fd, int id, uint32_t freq) {
  ...
  VC_MSG msg2 = {
      .tag = 0x38002, .blen = 12, .dlen = 12, .uints = {id, freq, 0}};
  msg_mbox(fd, &msg1);
  disp_vc_msg(&msg1);
  msg_mbox(fd, &msg2);
  disp_vc_msg(&msg2);
...
void disp_vc_msg(VC_MSG* msgp) {
  ...
  printf("VC msg len=%X, req=%X, tag=%X, blen=%x, dlen=%x, data ", msgp->len,
         msgp->req, msgp->tag, msgp->blen, msgp->dlen);
